...
variable "nsg_default_rules" {
  description = <<EOT
  (Optional) List of Default NSG Rules to be created. The nsg_default_rules object supports the following:
  ...
  default = null
  validation {
    condition = var.nsg_default_rules == null || alltrue(
      [for e in coalesce(var.nsg_default_rules, []) : length(regexall("^[a-zA-Z][a-zA-Z0-9_.-]{0,78}[a-zA-Z0-9_]$", e.name)) > 0]
    )
  ...
  validation {
    condition = var.nsg_default_rules == null || alltrue(
      [for e in coalesce(var.nsg_default_rules, []) : e.priority == 4000 || (e.priority >= 1000 && e.priority < 1800)]
    )
    error_message = "Default Rules priority must be between 1000 and 1799. The Deny All rule also permitted at priority 4000."
  }
}
