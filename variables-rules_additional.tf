...
variable "nsg_additional_rules" {
  description = <<EOT
  (Optional) List of Additional NSG Rules to be created. The nsg_additional_rules object supports the following:
  ...
  default = null
  validation {
    condition = var.nsg_additional_rules == null || alltrue(
      [for e in coalesce(var.nsg_additional_rules, []) : length(regexall("^[a-zA-Z][a-zA-Z0-9_.-]{0,78}[a-zA-Z0-9_]$", e.name)) > 0]
    )
  ...
  validation {
    condition = var.nsg_additional_rules == null || alltrue(
      [for e in coalesce(var.nsg_additional_rules, []) : e.priority >= 1800 && e.priority < 2000]
    )
    error_message = "Additional Rules priority must be between 1800 and 1999."
  }
}
